...
def convert_examples_to_features(examples, label_list, max_seq_length,
    tokenizer_label, tokenizer, thresholds, entity_id2parents, entity_id2label, max_parent,
    qid2idx, verbose=2):
...
def main():
    ...
    # The entity annotations are labled by tagme from ERNIE and we use the same threshould to filter unreliable annotations as ERNIE.
        ...
        for epoch in trange(int(args.num_train_epochs), desc="Epoch"):
            ...
            for step, batch in enumerate(tqdm(train_dataloader, desc="Iteration")):
                ...
                if (step + 1) % args.gradient_accumulation_steps == 0:
                    ...
                    optimizer.zero_grad()
                    global_step += 1
                    # Following ERNIE, record every 150 steps
                    if global_step % 150 == 0 and global_step > 0:
                        result = do_eval("dev", global_step)
                        logger.info(f"*****Dev result@step {global_step}: {result} *****")
