...
class SentenceSplitter:
    ...
    def get_sentence_spans(self, text: str) -> List[Tuple[int, int]]:
    ...
    def from_name(cls, name: str):
        ...
        if name == "opennlp":
            return OpenNLPSentenceSplitter()
        else:
            return ICUSentenceSplitter(name)


class ICUSentenceSplitter(SentenceSplitter):
    ...
    def get_sentence_spans(self, text: str):
        """
        ...
        len(text)
        >>> 6
        icu_tokenizer.get_sentence_spans(text)
        >>> [(0, 7)]
        ```
...
class OpenNLPSentenceSplitter(SentenceSplitter):
    ...
    def initialize(self):
        ...
        if not OpenNLPSentenceSplitter._java_initialized:
            import jnius_config

            jnius_config.add_options("-Xrs")
            # used to be /resources/opennlp-tools-1.5.3.jar
            # this adds the classpath "InsGenEntityLinking/src/common_utils/resources/apache-opennlp-1.5.3/lib/*"
