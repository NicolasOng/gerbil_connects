...
def ebert_model(raw_text):
    ...
    # offset them by the sentence's starting token index so the span
    # refer's to the entire input's tokens, not just the sentence's
    preds = []
    offset = 0
...
class GerbilAnnotator:
    ...
    def annotate(self, nif_collection, **kwargs):
        ...
        for annotation in ebert_model(raw_text):
